...
static void event_handler(void* arg, esp_event_base_t event_base, int32_t event_id, void* event_data)
{
    ...
    else if (event_base == IP_EVENT && event_id == IP_EVENT_STA_GOT_IP)
    {
        ...
        // Store ip
        _ip = malloc(17);
        snprintf(_ip, 17, IPSTR, IP2STR(&event->ip_info.ip));
...
bool wifi_init_sta(const char* ssid, const char* pass)
{
    ...
    while (1)
    {
        ...
        bits =
            xEventGroupWaitBits(s_wifi_event_group, WIFI_CONNECTED_BIT | WIFI_FAIL_BIT, pdFALSE, pdFALSE, pdMS_TO_TICKS(500));
        if (bits != 0)
            break;
    }
...
uint8_t wifi_start(const char* ssid, const char* pass)
{
    ...
    return wifi_init_sta(ssid, pass);
}

uint8_t wifi_is_connected() { return _is_wifi_connected; }

char* wifi_get_ip() { return _ip; }
