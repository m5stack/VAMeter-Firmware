 ...
 * It either allows the embedded application i.e. example to access the partition or Host PC accesses the partition over USB
 * MSC. They can't be allowed to access the partition at the same time. For different scenarios and behaviour, Refer to README
 * of this example.
 */
...
static int console_write(int argc, char** argv)
{
    ...
    if (!fd)
    {
        ...
        fd = fopen(filename, "w");
        fprintf(fd,
                "Mass Storage Devices are one of the most common USB devices. It use Mass Storage Class (MSC) that allow "
                "access to their internal data storage.\n");
        fprintf(fd, "In this example, ESP chip will be recognised by host (PC) as Mass Storage Device.\n");
        fprintf(fd,
                "Upon connection to USB host (PC), the example application will initialize the storage module and then the "
                "storage will be seen as removable device on PC.\n");
        fclose(fd);
    }
    return 0;
}
...
static esp_err_t storage_init_sdmmc(sdmmc_card_t** card)
{
    ...
    ESP_GOTO_ON_ERROR(
        sdmmc_host_init_slot(host.slot, (const sdmmc_slot_config_t*)&slot_config), clean, TAG, "Host init slot fail");
