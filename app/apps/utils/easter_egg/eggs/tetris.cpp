...
#define FADING_TIME 33
    ...
    static TetrisData_t* _data = nullptr;
    ...
    void start_tetris()
    {
        ...
        while (1)
        {
            ...
            if (HAL::Millis() - update_count > 20)
            {
                ...
                if (Button::Side()->wasHold() && _data->gameOver)
                    break;
    ...
    void UpdateGame(void)
    {
        if (!_data->gameOver)
        {
            ...
            // if (Button::Side()->wasDoubleClicked())
            ...
            if (!_data->pause)
            {
                if (!_data->lineToDelete)
                {
                    ...
                    else // Piece falling
                    {
                        ...
                        if (Button::Side()->isPressed() && (_data->fastFallMovementCounter >= FAST_FALL_AWAIT_COUNTER))
                        {
    ...
    void DrawGame(void)
    {
        ...
        if (!_data->gameOver)
        {
            ...
            HAL::GetCanvas()->setCursor(162, 150);
            HAL::GetCanvas()->printf("LINES");
            HAL::GetCanvas()->setCursor(166, 180);
